...
export function meta() {
  return [{ title: "Celebrity Hair Transplant" }];
}

export default function CelebrityTransplantEn() {
  const { slug } = useParams<{ slug: string }>();
  const path = slug ? `/${slug}` : "/";
  const resolved = resolvePage("en", path);

  if (!resolved || resolved.type !== "dynamic" || resolved.key !== "celebrity_hair_transplant_en") {
  ...
  return (
    <div style={page}>
      <div style={card}>
        <span style={badge}>EN · celebrity_hair_transplant_en</span>
        <h1 style={title}>{resolved.slug}</h1>
        <p style={sub}>
